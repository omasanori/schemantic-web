...
(define (test-uri-merging)
  (let ((base-uri (string->uri "http://a/b/c/d;p?q")))
    (define (test examples)
      (filter-map (lambda (example)
                    (let ((relative (car example))
                          (expected (cadr example)))
                      (let ((merged
                             (uri->string (merge-uris relative base-uri #t))))
                        (if (not (string=? merged expected))
                            `(,relative ,merged ,expected)
                            #f))))
                  examples))
    (let ((failures:normal (test test-uri-merging-examples:normal))
          (failures:abnormal (test test-uri-merging-examples:abnormal)))
      (if (and (null? failures:normal)
               (null? failures:abnormal))
          'PASS
          `(FAIL (NORMAL-EXAMPLES ,@failures:normal)
                 (ABNORMAL-EXAMPLES ,@failures:abnormal))))))

;;; Taken from the RFC, pages 35-37.

(define test-uri-merging-examples:normal
  ;; Format is (RELATIVE EXPECTED-MERGED).
  '(
    ("g:h"      "g:h")
    ("g"        "http://a/b/c/g")
    ("./g"      "http://a/b/c/g")
...
(define test-uri-merging-examples:abnormal
  '(
    ("../../../g"       "http://a/g")
    ("../../../../g"    "http://a/g")
    ("/./g"             "http://a/g")
